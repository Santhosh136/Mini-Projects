...
public class ParseFeed {
	...
	public static HashMap<String, String> loadImpactFromCSV(PApplet p, String fileName) {
		HashMap<String , String> impact = new HashMap<>();
		String rows[] = p.loadStrings(fileName);
		
		for(int i=1; i<rows.length; i++) {
			String[] columns = rows[i].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			if(!columns[0].equals("") && !columns[6].equals("")) {
				impact.put(columns[0], columns[6]);
			}
		}
	...
	public static HashMap<String, Feature> loadCovidDataFromCSV(PApplet p, String fileName) {
		HashMap<String, Feature> impacts = new HashMap<>();
		String rows[] = p.loadStrings(fileName);
		
		for(int i=1; i<rows.length; i++) {
			String[] columns = rows[i].split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			if(!columns[0].equals("") && !columns[6].equals("")) {
				double lat = Double.parseDouble(columns[12]);
				double lon = Double.parseDouble(columns[13]);
				Location location = new Location(lat, lon);
				
				PointFeature point = new PointFeature(location);
				point.addProperty("id", columns[0]);
				point.addProperty("country", columns[1]);
				point.addProperty("status", columns[6]);
				point.addProperty("income", columns[4]);
				point.addProperty("enrollment", columns[14]);
				impacts.put(columns[0],point);
